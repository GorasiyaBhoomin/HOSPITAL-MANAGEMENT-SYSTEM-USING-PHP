<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if($result->num_rows > 0) {
            ...
            try {
                // Insert user data
                $sql = "INSERT INTO users (username, email, mobile, dob, gender, address, password, status) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                
                $stmt = $con->prepare($sql);
                $status = 'inactive';
                $stmt->bind_param("ssssssss", $username, $email, $mobile, $dob, $gender, $address, $hashed_password, $status);

                if($stmt->execute()) {
                    $user_id = $con->insert_id;

                    // Insert activation data
                    $activation_sql = "INSERT INTO activation (user_id, activation_code, created_at, expires_at) 
                                     VALUES (?, ?, ?, ?)";
                    $stmt = $con->prepare($activation_sql);
                    $stmt->bind_param("isss", $user_id, $activation_token, $created_at, $expires_at);
                    ...
                    if($stmt->execute()) {
                        ...
                        $con->rollback();
                        echo "<script>alert('Failed to create activation record. Please try again.'); window.location.href='register.php';</script>";
                    }
                } else {
                    $con->rollback();
                    echo "<script>alert('Failed to create user account. Please try again.'); window.location.href='register.php';</script>";
                }
            } catch (Exception $e) {
                $con->rollback();
