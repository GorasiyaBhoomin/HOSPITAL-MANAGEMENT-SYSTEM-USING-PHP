...
if(isset($_GET['id'])) {
    ...
    if($doctor_stmt->execute()) {
        $_SESSION['doctor_deleted'] = "Doctor deleted successfully";
        header("Location: doctors.php");
        exit();
    } else {
        $_SESSION['doctor_deleted'] = "Error deleting doctor: " . $con->error;
        header("Location: doctors.php");
        exit();
    }
}
...
if(isset($_POST['doctor_id'])) {
    ...
    // Delete doctor's time slots first (foreign key constraint)
    ...
    $stmt = $con->prepare($delete_doctor);
    $stmt->bind_param("i", $doctor_id);
    ...
    <div class="page-wrapper">
        <div class="content">
            <div class="row">
                ...
                <?php
                // Display doctor deleted message if set
                if(isset($_SESSION['doctor_deleted'])) {
                    echo "<div class='col-12'><div class='alert alert-success'>".$_SESSION['doctor_deleted']."</div></div>";
                    unset($_SESSION['doctor_deleted']);
                }
                
                // Also check for delete_success/delete_error messages
                if(isset($_SESSION['delete_success'])) {
                    echo "<div class='col-12'><div class='alert alert-success'>".$_SESSION['delete_success']."</div></div>";
                    unset($_SESSION['delete_success']);
                }
                if(isset($_SESSION['delete_error'])) {
                    echo "<div class='col-12'><div class='alert alert-danger'>".$_SESSION['delete_error']."</div></div>";
                        ...
                        echo '<div class="col-md-4 col-sm-4 col-lg-3">
                                <div class="profile-widget">
                                    <div class="doctor-img">
                                        <a class="avatar" href="edit-doctor.php?id=' . $row['id'] . '">
                                            <img alt="" src="' . (!empty($row['avatar']) ? htmlspecialchars($row['avatar']) : 'assets/img/user.jpg') . '">
                                        </a>
                                    </div>
                                    <div class="dropdown profile-action">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            <a class="dropdown-item" href="edit-doctor.php?id=' . $row['id'] . '"><i class="fa fa-pencil m-r-5"></i> Edit</a>
                                            <form method="POST" action="doctors.php" style="display: inline;">
                                                <input type="hidden" name="doctor_id" value="' . $row['id'] . '">
                                                <button type="submit" class="dropdown-item" onclick="return confirm(\'Are you sure you want to delete this doctor?\');"><i class="fa fa-trash-o m-r-5"></i> Delete</button>
                                            </form>
                                        </div>
                                    </div>
                                    <h4 class="doctor-name text-ellipsis"><a href="profile.php?id=' . $row['id'] . '">' . htmlspecialchars($row['username']) . '</a></h4>
                                    <div class="doc-prof">' . htmlspecialchars($row['departmentname']) . '</div>
                                </div>
                              </div>';
                    }
                } else {
