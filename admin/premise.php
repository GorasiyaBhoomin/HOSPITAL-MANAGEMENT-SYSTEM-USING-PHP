...
// Fetch premises data from database
$sql = "SELECT * FROM premises ORDER BY created_at DESC";
$stmt = $con->prepare($sql);
$stmt->execute();
$result = $stmt->get_result();
$premises = $result->fetch_all(MYSQLI_ASSOC);

// Handle premise deletion
if(isset($_GET['id'])) {
    $premise_id = $_GET['id'];
    
    // Delete premise directly since there's no premises_usage table
    $delete_premise = "DELETE FROM premises WHERE id = ?";
    $stmt = $con->prepare($delete_premise);
    $stmt->bind_param("i", $premise_id);
    
    if($stmt->execute()) {
        $_SESSION['success'] = "Premise deleted successfully";
    } else {
        $_SESSION['error'] = "Error deleting premise: " . $con->error;
    }
        ...
        <div class="content">
            <div class="row">
                <div class="col-sm-4 col-3">
                    <h4 class="page-title">Premises</h4>
                </div>
                <div class="col-sm-7 col-7 text-right m-b-30">
                    <a href="add_premise.php" class="btn btn-primary btn-rounded"><i class="fa fa-plus"></i> Add Premises</a>
                </div>
            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped custom-table">
                            ...
                            <tr>
                                ...
                                <td><?= nl2br(htmlspecialchars($p['location'])) ?></td>
            ...
            <td class="text-right">
                                        <div class="dropdown dropdown-action">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                            <a class="dropdown-item" href="edit_premise.php?id=<?= $p['id'] ?>" class="button edit"><i class="fa fa-pencil m-r-5"></i>Edit</a>
                                            <a class="dropdown-item" href="premise.php?id=<?= $p['id'] ?>" onclick="return confirm('Are you sure you want to delete this premise?');"><i class="fa fa-trash-o m-r-5"></i> Delete</a>
                                            </div>
                                        </div>
                                    </td>
        </tr>
